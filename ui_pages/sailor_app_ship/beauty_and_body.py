...
class Beauty_and_body(General):
    ...
    def __init__(self, web_driver):
        ...
        self.locators = self.dict_to_ns({
                "beauty_and_body": "//*[@text='Beauty & Body']",
                "loader": "//*[@text='loading...']",
                "redemption_spa_section": "//h2[text()='Redemption Spa']",
    ...
    def verify_availability_of_beauty_and_body_header(self):
        ...
        if self.webDriver.is_element_display_on_screen(element=self.locators.beauty_and_body, locator_type='xpath'):
            logger.debug("Beauty and Body header is available on screen")
        else:
            raise Exception("Beauty and Body header is not available on screen")

    def open_redemption_spa(self):
        """
        Click on Redemption Spa
        :return:
        """
        self.webDriver.wait_till_element_appear_on_screen(element=self.locators.redemption_spa_section,
                                                          locator_type='xpath')
        self.webDriver.scroll_till_element(element=self.locators.redemption_spa_section, locator_type='xpath')
        self.webDriver.click(element=self.locators.redemption_spa_section, locator_type='xpath')
        self.webDriver.wait_till_element_disappear_from_screen(element=self.locators.redemption_spa_section,
                                                               locator_type='xpath')
    ...
    def book_resurrection_acupuncture(self, test_data):
        ...
        self.webDriver.wait_till_element_disappear_from_screen(element=self.locators.loading,
                                                               locator_type='xpath')
        if self.webDriver.is_element_display_on_screen(element=self.locators.choose_a_treatment_btn,
                                                       locator_type='xpath'):
