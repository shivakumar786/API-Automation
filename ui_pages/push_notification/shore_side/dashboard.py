...
class PushNotificationDashboard(General):
    ...
    def __init__(self, web_driver):
        ...
        self.locators = self.dict_to_ns({
            ...
            "select_ship": "//div[@class='MuiInputBase-root MuiFilledInput-root MuiFilledInput-underline MuiInputBase-formControl']",
            "ship_name": "//li[contains(text(),'%s')]",
            "draft_list": "//h5[contains(text(),'No Saved Drafts')]"        })
    ...
    def get_dashboard_count(self):
        ...
        :return:
        """
        self.webDriver.explicit_visibility_of_element(element=self.locators.notification_count, locator_type='xpath',
                                                      time_out=160)
    ...
    def get_scheduled_notification_count(self):
        ...
        :return:
        """
        self.webDriver.explicit_visibility_of_element(element=self.locators.schedule_count, locator_type="xpath",
                                                      time_out=60)
    ...
    def change_ship(self, ship):
        """
        To change the ship from ship dropdown
        :param ship:
        """
        self.webDriver.click(element=self.locators.select_ship, locator_type="xpath")
        self.webDriver.click(element=self.locators.ship_name % ship, locator_type="xpath")
        self.wait_for_loader_to_complete()

    def verify_draft_list(self):
        """
        To check whether draft section has any drafts or not
        """
        if not self.webDriver.is_element_display_on_screen(element=self.locators.draft_list, locator_type='xpath'):
            return True
        else:
            return False
